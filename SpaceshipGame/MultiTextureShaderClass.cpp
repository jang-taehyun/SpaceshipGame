...
MultiTextureShaderClass::MultiTextureShaderClass(const HWND& hwnd, ID3D11Device* const& Device, const ShaderFileInfo& info) : ShaderClass(hwnd, Device, info)
{
	HRESULT result = S_OK;

	// 에러 메세지 초기화 //
	e.title = _T("MultiTextureShaderClass constructor");
...
HRESULT MultiTextureShaderClass::CreateInputLayout(ID3D11Device* const& Device)
{
	HRESULT result = S_OK;
	...
	// 에러 메세지, 지역 변수 초기화
	e.title = _T("MultiTextureShaderClass CreateInputLayout()");
...
void MultiTextureShaderClass::ShutdownLayout()
{
	if (m_Layout)
	{
		m_Layout->Release();
