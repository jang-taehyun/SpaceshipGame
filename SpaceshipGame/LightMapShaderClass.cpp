...
{
	return InitializeShader(Device, hwnd, _T("../Engine/lightmap.vs"), _T("../Engine/lightmap.ps"));
}
...
bool LightMapShaderClass::Render(ID3D11DeviceContext* DeviceContext, int IndexCount, DirectX::XMMATRIX WorldMatrix, DirectX::XMMATRIX ViewMatrix, DirectX::XMMATRIX ProjectionMatrix, ID3D11ShaderResourceView** TextureArray)
{
...
bool LightMapShaderClass::InitializeShader(ID3D11Device* Device, HWND hwnd, const TCHAR* vsFileName, const TCHAR* psFileName)
{
	ID3D10Blob* ErrorMessage = nullptr;

	// vertex shader code 컴파일 //
	ID3D10Blob* VertexShaderBuffer = nullptr;
	if (FAILED(D3DCompileFromFile(vsFileName, NULL, NULL, "LightMapVertexShader", "vs_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, &VertexShaderBuffer, &ErrorMessage)))
	{
		if (ErrorMessage)
		{
	...
	if (FAILED(D3DCompileFromFile(psFileName, NULL, NULL, "LightMapPixelShader", "ps_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, &PixelShaderBuffer, &ErrorMessage)))
	{
		if (ErrorMessage)
		{
...
void LightMapShaderClass::OutputShaderErrorMessage(ID3D10Blob* ErrorMessage, HWND hwnd, const TCHAR* ShaderFileName)
{
...
bool LightMapShaderClass::SetShaderParameters(ID3D11DeviceContext* DeviceContext, DirectX::XMMATRIX WorldMatrix, DirectX::XMMATRIX ViewMatrix, DirectX::XMMATRIX ProjectionMatrix, ID3D11ShaderResourceView** TextureArray)
{
	// 행렬들을 HLSL에 맞게 변환 //
